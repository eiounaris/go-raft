...
func Make(peers []peer.Peer, me int, persister *persister.Persister, applyCh chan ApplyMsg) *Raft {
	rf := &Raft{
		mu:             sync.RWMutex{},
		peers:          peers,
		persister:      persister,
...
func (rf *Raft) Snapshot(index int, snapshot []byte) {
	...
	if index <= snapshotIndex || index > rf.getLastLog().Index {
		util.DPrintf("{Node %v} rejects replacing log with snapshotIndex %v as current snapshotIndex %v is larger in term %v", rf.me, index, snapshotIndex, rf.currentTerm)
		return
	}
	...
	util.DPrintf("{Node %v}'s state is {state %v,term %v,commitIndex %v,lastApplied %v,firstLog %v,lastLog %v} after accepting the snapshot with index %v", rf.me, rf.state, rf.currentTerm, rf.commitIndex, rf.lastApplied, rf.getFirstLog(), rf.getLastLog(), index)
}
...
func (rf *Raft) CondInstallSnapshot(lastIncludedTerm int, lastIncludedIndex int, snapshot []byte) bool {
	...
	if lastIncludedIndex <= rf.commitIndex {
		util.DPrintf("{Node %v} rejects outdated snapshot with lastIncludeIndex %v as current commitIndex %v is larger in term %v", rf.me, lastIncludedIndex, rf.commitIndex, rf.currentTerm)
		return false
	}
	...
	util.DPrintf("{Node %v}'s state is {state %v,term %v,commitIndex %v,lastApplied %v,firstLog %v,lastLog %v} after accepting the snapshot which lastIncludedTerm is %v, lastIncludedIndex is %v", rf.me, rf.state, rf.currentTerm, rf.commitIndex, rf.lastApplied, rf.getFirstLog(), rf.getLastLog(), lastIncludedTerm, lastIncludedIndex)
	return true
}
...
func (rf *Raft) replicateOnceRound(peer int) {
	...
	if prevLogIndex < rf.getFirstLog().Index {
		...
		if rf.sendInstallSnapshot(peer, args, reply) {
			util.DPrintf("{Node %v} sends InstallSnapshotArgs %v to {Node %v} and receives InstallSnapshotReply %v", rf.me, args, peer, reply)
			rf.mu.Lock()
	...
	case Leader:
		...
		for peer := range rf.peers {
			if peer != rf.me {
				...
				rf.matchIndex[peer], rf.nextIndex[peer] = rf.getLastLog().Index, rf.getLastLog().Index+1
			}
		}
...
func (rf *Raft) readPersist(data []byte) {
	...
	if d.Decode(&currentTerm) != nil || d.Decode(&votedFor) != nil || d.Decode(&logs) != nil {
		util.DPrintf("{Node %v} fails to decode persisted state", rf.me)
	}
